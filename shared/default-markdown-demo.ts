import { DEFAULT_IMPRESSUM_DATA } from "./impressum-default-data.js";

/** `{{impressumField key=value}}` — value must not contain `}` (macro syntax). */
...
/**
 * Default markdown on app start: all macros + typical Markdown elements.
 * **When adding macros:** extend with a short Lorem example here; add new \`\\page\` sections as needed.
 */
