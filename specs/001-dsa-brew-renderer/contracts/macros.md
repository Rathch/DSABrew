...
- **NSC-Vorlage** (`npc-block-template.png` aus `media/foo.png`): Nach dem Kopieren setzt `web/scripts/punch-npc-portrait-hole.mjs` (in `npm run prepare-assets`) die Porträt-**Mitte** per Kreis (an die CSS-Prozente `--npc-p-*` gekoppelt) auf **transparent**, damit das Portrait darunter sichtbar wird.
- **NSC-Vorlage**: Bevorzugt **`media/npc-block-template.png`** (Porträt-Bereich mit Alpha) → `web/public/dsa/npc-block-template.png`. Fehlt die Datei, Fallback **`media/foo.png`**; dann erzeugt `punch-npc-portrait-hole.mjs` beim **`npm run prepare-assets`** ein transparentes Loch (gleiche CSS-Prozente `--npc-p-*`).
